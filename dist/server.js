...
async function getTweetsFromZip() {
    const zip = new node_stream_zip_1.default.async({ file: zipFilePath });
    try {
        const entry = await zip.entry('data/tweets.js');
        if (!entry)
            throw new Error('data/tweets.js not found in ZIP');
...
server.resource('tweet-list', 'tweet-list://recent', async () => {
...
// Resource: tweet://{id}
server.resource('tweet', new mcp_js_1.ResourceTemplate('tweet://{id}', { list: undefined }), async (uri, { id }) => {
    const tweets = await getTweetsFromZip();
...
server.resource('tweet-text', new mcp_js_1.ResourceTemplate('tweet-text://{id}', { list: undefined }), async (uri, { id }) => {
    const tweets = await getTweetsFromZip();
    const tweet = tweets.find((t) => (t.id_str || t.id) === id);
    if (!tweet) {
        return {
...
server.tool('sample_tweet_texts', { sampleSize: zod_1.z.string().optional() }, async ({ sampleSize }) => {
    let n = 5;
    if (sampleSize) {
